	...
	<table class="table table-bordered tablesorter table-striped">
		<thead>
			...
			<?php foreach($columns as $column){?>
				...
				<th class="no-sorter"><?php echo $this->l('list_actions'); ?>
				</th>
				<?php }?>
			</tr>
		</thead>
						...
						<div class="btn-group">
							...
							if(!$unset_edit){?>
								<li><a href="<?php echo $row->edit_url?>"
									title="<?php echo $this->l('list_edit')?> <?php echo $subject?>">
										<i class="icon-pencil"></i> <?php echo $this->l('list_edit') . ' ' . $subject; ?>
								</a>
								</li>
								<?php
							}
							if(!$unset_delete){?>
								<li><a href="javascript:void(0);"
									...
									class="delete-row"> <i class="icon-trash"></i> <?php echo $this->l('list_delete') . ' ' . $subject; ?>
								</a>
								</li>
								<?php
							}
							if(!empty($row->action_urls)){
								...
								<li><a href="<?php echo $action_url; ?>"
									...
									if(!empty($action->image_url)){ ?> <img
										src="<?php echo $action->image_url; ?>" alt="" /> <?php
									}
									echo ' '.$action->label;
									?> </a>
								</li>
