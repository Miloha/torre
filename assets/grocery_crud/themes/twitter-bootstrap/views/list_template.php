	...
	<div id="main-table-box">
		...
		<?php if(!$unset_add){?>
			<a href="<?php echo $add_url?>"
				...
				class="add-anchor btn"> <i class="icon-plus"></i> <?php echo $this->l('list_add'); ?>
				<?php echo $subject?> </a>
				<?php
		}
		if(!$unset_export) { ?>
			<a class="export-anchor btn" data-url="<?php echo $export_url; ?>"
				rel="external"> <i class="icon-download"></i> <?php echo $this->l('list_export');?>
			</a>
			<?php
		}
		if(!$unset_print) { ?>
			<a class="print-anchor btn" data-url="<?php echo $print_url; ?>"> <i
				class="icon-print"></i> <?php echo $this->l('list_print');?> </a>
				<?php
		}
		} ?>
			<a class="btn" data-toggle="modal" href="#filtering-form-search"> <i
				class="icon-search"></i> <?php echo $this->l('list_search');?> </a>
		</div>
		<br />
		...
		<div class="pGroup span12">
			...
			<?php foreach($paging_options as $option){?>
				...
				<?php echo ($option == $default_per_page) ? 'selected="selected"' : ''; ?>>
					<?php echo $option; ?>
				</option>
				<?php }?>
			</select> <span class="pPageStat"> <?php
			...
			</span> <span class="pcontrol"> <?php echo $this->l('list_page'); ?>
				<input name="tb_crud_page" type="text" value="1" size="4"
				id="tb_crud_page"> <?php echo $this->l('list_paging_of'); ?> <span
				id="last-page-number"><?php echo ceil($total_results / $default_per_page); ?>
			</span> </span>

			<div class="hide loading" id="ajax-loading">
			<?php echo $this->l('form_update_loading'); ?>
			</div>

			<ul class="pager">
				<li class="previous first-button"><a href="javascript:void(0);">&laquo;
				<?php echo $this->l('list_paging_first'); ?> </a></li>
				<li class="prev-button"><a href="javascript:void(0);">&laquo; <?php echo $this->l('list_paging_previous'); ?>
				</a></li>
				<li class="next-button"><a href="javascript:void(0);"><?php echo $this->l('list_paging_next'); ?>
						&raquo;</a></li>
				<li class="next last-button"><a href="javascript:void(0);"><?php echo $this->l('list_paging_last'); ?>
						&raquo;</a></li>
			</ul>
		</div>
	</div>
...
<div class="modal hide" id="filtering-form-search">
	<div class="modal-header">
		...
		<h3>
		<?php echo $this->l('list_search') . ' ' . $subject; ?>
		</h3>
	</div>
					...
					<?php echo form_open( $ajax_list_url, 'method="post" id="filtering_form" autocomplete = "off"'); ?>
						<div class="sDiv" id="quickSearchBox">
							<div class="sDiv2">
								...
								: <input type="text" class="qsbsearch_fieldox"
									name="search_text" size="30" id="search_text"> <select
