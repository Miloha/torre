...
<style type="text/css">
<?php
 foreach ($actions as  $action_unique_id  => $action ){ ?> <?php
	if(!empty($action->image_url)){ ?> .<?php echo $action_unique_id; ?>{
...
.datatables-add-button {
	...
	<?php }?>
<div
	id='list-report-error' class='report-div error report-list'></div>
...
<div class="datatables-add-button">
	<a role="button"
		...
		class="ui-button-text"><?php echo $this->l('list_add'); ?> <?php echo $subject?>
	</span> </a>
</div>
	<?php } ?>
